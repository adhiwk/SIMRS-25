    ...
    public class BpjsLZString
    {
        ...
        private static string Compress(string uncompressed, int bitsPerChar, Func<int, char> getCharFromInt)
        {
            ...
            if (context_w != "")
            {
                ...
                if (context_enlargeIn == 0)
                {
                    // context_enlargeIn = (int)Math.Pow(2, context_numBits); // Unused.
                    // Kept for tracking changes with https://github.com/pieroxy/lz-string/blob/master/libs/lz-string.js#L295
                    context_numBits++;
                }
            }
